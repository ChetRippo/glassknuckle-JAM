...
function OnGUI() {
	if (GetComponent(date).currentDate == animalName){
		action = true;
		if (playerObject.GetComponent(player).levelMercedes == 0){
			...
			if (GetComponent(date).pos == 20) {
				...
				playerObject.GetComponent(player).levelMercedes = 1;
				Application.LoadLevel ("map");
			}
		}else if (playerObject.GetComponent(player).levelMercedes == 1){
			if (GetComponent(date).pos == 0){
				GetComponent(date).vocal = "Honk. Honk….?";
			}
			if (GetComponent(date).pos == 1) {
			...
			if (GetComponent(date).pos == 20) {
				...
				playerObject.GetComponent(player).levelMercedes = 2;
				Application.LoadLevel("map");
			}
		}
	}
